...
import { asYen, blindChoicesFor, calendarDayTotal, currencies, dateLabel, duration, fromMinor, hours, money, profit, sessionFormat, sessionRoi, shortDate, toMinor, tripsForDate, validSessionTimeRange, type Currency, type Player, type Rate, type Session, type Trip } from './domain'
...
const nav: { page: Page; label: string; icon: typeof LayoutDashboard }[] = [
  ...
  { page: 'trips', label: '遠征収支', icon: Plane },
  { page: 'calendar', label: 'カレンダー', icon: CalendarDays },
  { page: 'analytics', label: '収支分析', icon: BarChart3 },
  { page: 'players', label: 'テーブルメモ', icon: Users },
  { page: 'settings', label: '設定・データ', icon: Settings2 },
]
const title: Record<Page, string> = { overview: 'ダッシュボード', sessions: 'セッション', trips: '遠征収支', calendar: 'カレンダー', analytics: '収支分析', players: 'テーブルメモ', settings: '設定・データ' }
...
function App() {
  ...
  useEffect(() => { if (!user) localStorage.setItem('riverlog-demo-trips', JSON.stringify(trips)) }, [trips, user])
  ...
  const updateSessionInPlace = async (session: Session, successMessage?: string) => {
    ...
    setSelectedSession(current => current?.id === session.id ? session : null)
    if (user) try { await saveSessionRemote(user.uid, session) } catch { setSessions(previous); setSelectedSession(current => current?.id === session.id ? previous.find(item => item.id === session.id) || null : current); setNotice('テーブルメモの保存に失敗しました'); return }
    if (successMessage) setNotice(successMessage)
  }
  ...
  const removeTrip = async (id: string) => {
    ...
    const previous = trips; setTrips(current => current.filter(item => item.id !== id))
  ...
  const exportCsv = () => {
    const rows = [['id','tripId','format','venue','location','game','blindsOrTournament','currency','startedAt','endedAt','buyIn','rebuy','prizeOrCashOut','tips','profit','rate','rateDate','note'], ...sessions.map(s => [s.id,s.tripId || '',sessionFormat(s),s.venue,s.location,s.game,s.stakes,s.currency,s.startedAt,s.endedAt || '',fromMinor(s.buyIn,s.currency),fromMinor(s.rebuy,s.currency),fromMinor(s.cashOut,s.currency),fromMinor(s.tips,s.currency),fromMinor(profit(s),s.currency),s.rate?.rate || '',s.rate?.date || '',s.note])]
    downloadCsv(rows, `riverlog-sessions-${today()}.csv`)
  }
  const exportTripCsv = () => {
    const rows = [['tripId','tripName','destination','tripStart','tripEnd','expenseId','category','paidAt','currency','amount','rateToJPY','rateDate','note'], ...trips.flatMap(trip => trip.expenses.map(expense => [trip.id,trip.name,trip.destination,trip.startDate,trip.endDate,expense.id,expense.category,expense.spentAt,expense.currency,fromMinor(expense.amount,expense.currency),expense.rate?.rate || '',expense.rate?.date || '',expense.note]))]
    downloadCsv(rows, `riverlog-trip-expenses-${today()}.csv`)
  }
    ...
    <main className="main">
      ...
      <div className="content">
        ...
        {page === 'overview' && <>
          ...
          {active && <section className="active-session"><button className="active-session-summary" onClick={() => setSelectedSession(active)}><span className="active-label"><span className="pulse" /> {sessionFormat(active) === 'tournament' ? 'LIVE TOURNAMENT' : 'LIVE SESSION'}</span><h3>{active.venue} <span>{active.stakes}</span></h3><p>{duration(active, now)} プレー中 · {money(active.buyIn + active.rebuy, active.currency)} 投入</p></button><div className="active-actions"><button className="active-quick-button" onClick={() => addMatchingRebuy(active)}><Plus size={17} /><span>{sessionFormat(active) === 'tournament' ? '同額で再エントリー' : '同額リバイ'}<small>{money(active.buyIn, active.currency)}</small></span></button><button className="active-quick-button" onClick={() => setSessionAction({ kind: 'rebuy', session: active })}><Wallet size={17} /><span>別の金額<small>自由入力</small></span></button><button className="active-finish-button" onClick={() => setSessionAction({ kind: 'finish', session: active })}>終了する <ArrowRight size={16} /></button></div></section>}
          ...
          <div className="lower-grid"><section className="panel"><SectionHeading label="CURRENCY NOTE" title="円換算について" /><div className="rate-panel"><div className="rate-icon"><Globe2 size={23} /></div><div className="rate-values"><strong>{liveRate ? `1 USD ≈ ¥${liveRate.rate.toFixed(2)}` : 'USD レートを取得できませんでした'}</strong><strong>{wonRate ? `100 KRW ≈ ¥${(wonRate.rate * 100).toFixed(2)}` : 'KRW レートを取得できませんでした'}</strong><p>{liveRate || wonRate ? `${liveRate?.date || wonRate?.date} 基準 · Frankfurter の日次参考レート` : '現地通貨の記録は引き続き利用できます。'}</p></div></div><p className="panel-note">記録時のレートを固定保存します。実際の両替・決済レートとは異なります。</p></section><section className="panel"><SectionHeading label="QUICK ACCESS" title="次の記録へ" /><button className="quick-link" onClick={() => navigate('trips')}><span className="quick-icon"><Plane size={20} /></span><span><strong>遠征全体の収支を見る</strong><small>ポーカー収支から宿泊・食事・交通費を引く</small></span><ArrowRight size={18} /></button><button className="quick-link" onClick={() => navigate('players')}><span className="quick-icon"><Users size={20} /></span><span><strong>テーブルメモを開く</strong><small>{active ? `${active.venue}の卓を操作` : 'セッションを始めて卓を記録'}</small></span><ArrowRight size={18} /></button><button className="quick-link" onClick={() => navigate('calendar')}><span className="quick-icon"><CalendarDays size={20} /></span><span><strong>カレンダーを開く</strong><small>旅のプレー履歴を日付で探す</small></span><ArrowRight size={18} /></button></section></div>
        </>}
        ...
        {page === 'players' && <><div className="page-heading"><div><div className="eyebrow">LIVE TABLE</div><h1>テーブルメモ<span className="title-period">.</span></h1><p>プレー中の卓、相手、ポジションをここでまとめて操作する。</p></div>{active && <button className="secondary-button" onClick={() => setSelectedSession(active)}><Clock3 size={17} /> セッション操作</button>}</div>{active ? <div className="standalone-table"><div className="standalone-table-session"><span className="pulse" /><div><small>{sessionFormat(active) === 'tournament' ? 'LIVE TOURNAMENT' : 'LIVE CASH GAME'}</small><strong>{active.venue} · {active.stakes || 'ブラインド未設定'}</strong></div><em>{duration(active, now)} プレー中</em></div><PokerTable session={active} savedPlayers={players} onChange={(session, message) => void updateSessionInPlace(session, message)} onSavePlayer={player => void saveTablePlayer(player)} /></div> : <section className="table-empty-state"><div className="table-empty-icon"><Spade size={29} /></div><div><div className="eyebrow">NO ACTIVE TABLE</div><h2>セッションを始めると卓が開きます</h2><p>キャッシュゲームでもトーナメントでも、開始後すぐに席と相手を記録できます。</p></div><button className="primary-button" onClick={startOrOpenSession}><Plus size={17} /> セッションを開始</button></section>}{players.length > 0 && <section className="section saved-opponents"><SectionHeading label="SAVED OPPONENTS" title="記録した相手" /><div className="filter-bar"><div className="search-box"><Search size={18} /><input placeholder="名前・会場・タグで検索" value={search} onChange={e => setSearch(e.target.value)} /></div><span>{players.length} 人のメモ</span></div><div className="player-grid">{players.filter(p => `${p.name} ${p.venue} ${p.tags.join(' ')} ${p.note}`.toLowerCase().includes(search.toLowerCase())).map(p => <button className="player-card" key={p.id} onClick={() => setPlayerModal(p)}><div className="player-card-top"><div className="player-avatar">{p.name.slice(0, 1).toUpperCase()}</div><MoreHorizontal size={21} /></div><h3>{p.name}</h3><p><Globe2 size={14} /> {p.venue || '会場未設定'}</p><div className="tags">{p.tags.map(tag => <span key={tag}>{tag}</span>)}</div><div className="player-note">{p.note || 'メモはまだありません'}</div><small>更新 {shortDate(p.updatedAt)}</small></button>)}</div></section>}</>}
...
function SessionRow({ session, onClick }: { session: Session; onClick: () => void }) { const result = profit(session); const yen = asYen(result, session.currency, session.rate); const active = !session.endedAt; return <button className="session-row" onClick={onClick}><div className={`result-icon ${active ? 'running' : result < 0 ? 'loss' : ''}`}>{active ? <Clock3 size={19} /> : result < 0 ? <ArrowDownRight size={20} /> : <ArrowUpRight size={20} />}</div><div className="session-main"><strong>{session.venue}</strong><span>{session.stakes || (sessionFormat(session) === 'tournament' ? 'トーナメント' : 'ブラインド未設定')} <span className="separator">·</span> {dateLabel((session.localDate || session.startedAt.slice(0, 10)) + 'T12:00:00')}</span></div><div className="session-extra"><span>{sessionFormat(session) === 'tournament' ? 'トーナメント' : session.game}</span><small>{active ? 'プレー中' : duration(session)}</small></div><div className={`session-result ${active ? 'running' : result < 0 ? 'negative' : 'positive'}`}><strong>{active ? money(session.buyIn + session.rebuy, session.currency) : money(result, session.currency, true)}</strong><small>{active ? '投入中 · タップで操作' : formatYen(yen, true)}</small></div><ChevronRight size={17} className="row-chevron" /></button> }
...
function SessionForm({ initial, defaultLocation, defaultDate, tripId, tripName, sessions, rate, onClose, onSave }: { initial?: Session; defaultLocation?: string; defaultDate?: string; tripId?: string; tripName?: string; sessions: Session[]; rate: Partial<Record<Currency, Rate>>; onClose: () => void; onSave: (session: Session) => void }) {
  ...
  const [format, setFormat] = useState<'cash' | 'tournament'>(initial?.format || 'cash')
  const [stakes, setStakes] = useState(initial?.stakes || '')
  const [customStakes, setCustomStakes] = useState(initial?.format === 'tournament' || Boolean(initial?.stakes && !blindChoicesFor(initial.currency).includes(initial.stakes)))
  ...
  const [endedAt, setEndedAt] = useState(initial?.endedAt ? localInputDate(new Date(initial.endedAt)) : '')
  ...
  return <>
    <div className="modal-backdrop" onMouseDown={onClose}><div className="modal session-entry-modal" onMouseDown={event => event.stopPropagation()} role="dialog" aria-modal="true" aria-label={initial ? 'セッションを編集' : 'セッションを開始'}>
      ...
      <form onSubmit={submit}>
        ...
        <div className="field"><span>プレー形式</span><div className="choice-row"><button type="button" className={`choice-button ${format === 'cash' ? 'selected' : ''}`} aria-pressed={format === 'cash'} onClick={() => { setFormat('cash'); setStakes(''); setCustomStakes(false) }}>キャッシュゲーム</button><button type="button" className={`choice-button ${format === 'tournament' ? 'selected' : ''}`} aria-pressed={format === 'tournament'} onClick={() => { setFormat('tournament'); setStakes(''); setCustomStakes(true) }}>トーナメント</button></div></div>
        ...
        {format === 'cash' ? <div className="field"><span>ブラインド <small className="field-inline-hint">{currency}のよく使う金額</small></span><div className="choice-row stakes-choices">{stakeChoices.map(value => <button type="button" key={value} className={`choice-button ${stakes === value && !customStakes ? 'selected' : ''}`} aria-pressed={stakes === value && !customStakes} onClick={() => { setStakes(value); setCustomStakes(false) }}>{value}</button>)}<button type="button" className={`choice-button ${customStakes ? 'selected' : ''}`} aria-pressed={customStakes} onClick={() => { setStakes(''); setCustomStakes(true) }}>その他</button></div>{customStakes && <input value={stakes} onChange={event => setStakes(event.target.value)} maxLength={80} placeholder="例: 10-20、0.5-1" />}</div> : <label className="field full tournament-name-field">トーナメント名 <input value={stakes} onChange={event => setStakes(event.target.value)} maxLength={120} placeholder="例: Daily Deepstack" /></label>}
        <div className="entry-section-heading">日時と投入額</div>
        <div className="form-grid"><label className="field">開始日時 <input type="datetime-local" value={startedAt} onChange={event => setStartedAt(event.target.value)} required /></label>{completed ? <label className="field">終了日時 <input type="datetime-local" value={endedAt} min={startedAt} onChange={event => setEndedAt(event.target.value)} required />{!timeRangeValid && <small className="field-error">開始日時より後を指定してください。</small>}</label> : <label className="field buy-in-field">{format === 'tournament' ? '1エントリーのバイイン' : '初回バイイン'} <span className="currency-input"><b>{currency}</b><input type="number" inputMode="decimal" min="0" step="any" required value={buyIn} onChange={event => setBuyIn(event.target.value)} placeholder={currency === 'KRW' ? '300000' : currency === 'USD' ? '300' : '0'} /></span>{buyIn !== '' && <span className="yen-estimate" role="status" aria-live="polite"><span>現在の参考円換算</span><strong>{buyInYen === null ? 'レート取得中…' : `約 ${formatYen(buyInYen)}`}</strong>{currency !== 'JPY' && entryRate && <small>{rateUnit.toLocaleString('ja-JP')} {currency} ≈ ¥{(entryRate.rate * rateUnit).toLocaleString('ja-JP', { maximumFractionDigits: 2 })} · {entryRate.date}基準{entryRate.stale ? ' · 前回取得' : ''}</small>}</span>}</label>}</div>
        {completed && <label className="field buy-in-field">{format === 'tournament' ? '1エントリーのバイイン' : '初回バイイン'} <span className="currency-input"><b>{currency}</b><input type="number" inputMode="decimal" min="0" step="any" required value={buyIn} onChange={event => setBuyIn(event.target.value)} /></span>{buyIn !== '' && <span className="yen-estimate" role="status" aria-live="polite"><span>現在の参考円換算</span><strong>{buyInYen === null ? 'レート取得中…' : `約 ${formatYen(buyInYen)}`}</strong></span>}</label>}
        {completed && <><div className="entry-section-heading">確定済みの収支</div><div className="form-grid"><label className="field">{format === 'tournament' ? '獲得賞金' : 'キャッシュアウト'} <input type="number" inputMode="decimal" min="0" step="any" required value={cashOut} onChange={event => setCashOut(event.target.value)} /></label><label className="field">{format === 'tournament' ? '追加費用' : 'チップ'} <input type="number" inputMode="decimal" min="0" step="any" value={tips} onChange={event => setTips(event.target.value)} /></label></div>{previewProfit !== null && <div className={`entry-profit-preview ${previewProfit < 0 ? 'loss' : ''}`}><span>修正後の収支</span><strong>{money(toMinor(previewProfit, currency), currency, true)}</strong><small>{previewYen === null ? '円換算レート未取得' : `参考円換算 ${formatYen(previewYen, true)}`}</small></div>}</>}
        ...
        <div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>キャンセル</button><button className="primary-button" type="submit" disabled={!timeRangeValid}><Check size={17} /> {initial ? '変更を保存' : 'セッションを開始'}</button></div>
      </form>
    </div></div>
...
function RebuyForm({ session, onClose, onSave }: { session: Session; onClose: () => void; onSave: (amount: number) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal action-modal" onMouseDown={event => event.stopPropagation()} role="dialog" aria-modal="true" aria-label={tournament ? '再エントリーを追加' : 'リバイを追加'}>
    <div className="action-modal-head"><div className="action-icon"><Plus size={22} /></div><div><div className="eyebrow">{tournament ? 'RE-ENTRY' : 'ADD REBUY'}</div><h2>{tournament ? '再エントリー' : 'リバイを追加'}</h2><p>{session.venue} · {session.stakes}</p></div><button type="button" className="icon-button" onClick={onClose} aria-label="閉じる"><X size={21} /></button></div>
    ...
    <div className="money-context"><span>現在の投入合計</span><strong>{money(session.buyIn + session.rebuy, session.currency)}</strong><small>初回 {money(session.buyIn, session.currency)} · {tournament ? '再エントリー' : 'リバイ'} {money(session.rebuy, session.currency)}</small></div>
    ...
    <div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>戻る</button><button type="button" className="primary-button action-primary" disabled={minor <= 0} onClick={() => onSave(minor)}><Plus size={17} /> {tournament ? '再エントリー' : 'リバイを追加'}</button></div>
  </div></div>
}
...
function FinishSessionForm({ session, rate, onClose, onSave }: { session: Session; rate?: Rate; onClose: () => void; onSave: (session: Session) => void }) {
  ...
  const [endedAt, setEndedAt] = useState(() => localInputDate(new Date()))
  ...
  const endTimeValid = Boolean(endedAt) && validSessionTimeRange(session.startedAt, endedAt)
  const endTimeIso = endTimeValid ? new Date(endedAt).toISOString() : undefined
  const adjustedDuration = endTimeIso ? duration({ ...session, endedAt: endTimeIso }) : '—'
  const finish = (event: React.FormEvent) => {
    event.preventDefault()
    if (cashOutMinor === null || cashOutMinor < 0 || tipsMinor < 0 || !endTimeIso) return
    const now = new Date().toISOString()
    onSave({ ...session, endedAt: endTimeIso, cashOut: cashOutMinor, tips: tipsMinor, rate, updatedAt: now })
  }
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal action-modal finish-modal" onMouseDown={event => event.stopPropagation()} role="dialog" aria-modal="true" aria-label="セッションを終了">
    ...
    <div className="money-context"><span>投入した合計</span><strong>{money(invested, session.currency)}</strong><small>初回 {money(session.buyIn, session.currency)} · {tournament ? '再エントリー' : 'リバイ'} {money(session.rebuy, session.currency)}</small></div>
    <form onSubmit={finish}><div className="action-section"><label className="finish-time-field"><span>終了日時</span><input type="datetime-local" value={endedAt} min={localInputDate(new Date(session.startedAt))} onChange={event => setEndedAt(event.target.value)} required /><small className={endTimeValid ? '' : 'field-error'}>{endTimeValid ? `プレー時間 ${adjustedDuration}` : '開始日時より後を指定してください。'}</small></label><span className="action-label">{tournament ? '獲得賞金' : '手元に戻った金額'}</span><div className="finish-shortcuts"><button type="button" onClick={() => setCashOut(String(fromMinor(invested, session.currency)))}>±0 の金額</button><button type="button" onClick={() => setCashOut('0')}>{tournament ? 'ノーキャッシュ' : '全額失った'}</button></div><label className="money-input featured"><span>{session.currency}</span><input autoFocus required type="number" inputMode="decimal" min="0" step="any" value={cashOut} onChange={event => setCashOut(event.target.value)} placeholder={tournament ? '獲得賞金' : 'キャッシュアウト'} /></label><label className="compact-money-field"><span>{tournament ? '追加費用' : 'チップ・諸経費'}</span><input type="number" inputMode="decimal" min="0" step="any" value={tips} onChange={event => setTips(event.target.value)} /></label></div>
      <div className={`settlement-preview ${result !== null && result < 0 ? 'loss' : ''}`}><span>今回の収支</span><strong>{result === null ? '—' : money(result, session.currency, true)}</strong><small>{result === null ? `${tournament ? '獲得賞金' : 'キャッシュアウト'}を入力すると即時に計算します` : yen === null ? '円換算レート未取得' : `参考円換算 ${formatYen(yen, true)}`}</small></div>
      <p className="form-hint">{rate ? `${rate.date} 基準の参考レートをこの記録に固定します。` : '元通貨の収支は保存できます。円換算は未確定です。'}</p>
      <div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>まだ続ける</button><button className="primary-button action-primary" type="submit" disabled={cashOutMinor === null || !endTimeValid}><Check size={17} /> 収支を確定する</button></div></form>
  </div></div>
}
...
function SessionDetail({ session, players, currentRate, now, onUpdateSession, onSavePlayer, onClose, onEdit, onDelete, onQuickRebuy, onCustomRebuy, onFinish }: { session: Session; players: Player[]; currentRate?: Rate; now: number; onUpdateSession: (session: Session, message?: string) => void; onSavePlayer: (player: Player) => void; onClose: () => void; onEdit: () => void; onDelete: () => void; onQuickRebuy: () => void; onCustomRebuy: () => void; onFinish: () => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className={`modal detail-modal ${active ? 'live-detail-modal' : ''}`} onMouseDown={event => event.stopPropagation()} role="dialog" aria-modal="true" aria-label="セッション詳細">
    <div className="modal-header"><div><div className="eyebrow">{tournament ? active ? 'LIVE TOURNAMENT' : 'TOURNAMENT RESULT' : active ? 'LIVE SESSION' : 'SESSION RESULT'}</div><h2>{session.venue}</h2><span className="detail-subtitle">{session.stakes || (tournament ? 'トーナメント' : 'ブラインド未設定')} · {session.location || session.game} · {dateLabel((session.localDate || session.startedAt.slice(0, 10)) + 'T12:00:00')}</span></div><button className="icon-button" onClick={onClose} aria-label="閉じる"><X size={21} /></button></div>
    {active ? <>
      ...
      <div className="live-command-grid">
        <button className="live-command quick" onClick={onQuickRebuy}><span className="live-command-icon"><Plus size={20} /></span><span><small>ワンタップ</small><strong>{tournament ? '同額で再エントリー' : '同額リバイ'}</strong><em>{money(session.buyIn, session.currency)}</em></span></button>
      ...
      <PokerTable session={session} savedPlayers={players} onChange={onUpdateSession} onSavePlayer={onSavePlayer} />
    </> : <>
