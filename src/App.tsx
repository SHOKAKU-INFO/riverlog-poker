...
import { asYen, blindChoicesFor, calendarDayTotal, currencies, dateLabel, duration, fromMinor, hours, money, profit, samplePlayers, sampleSessions, sampleTrips, sessionFormat, sessionRoi, shortDate, toMinor, tournamentAverageEv, tournamentAverageReturn, tripsForDate, type Currency, type Player, type Rate, type Session, type Trip } from './domain'
...
function App() {
  ...
  const exportCsv = () => {
    const rows = [['id','tripId','format','venue','location','game','blindsOrTournament','entrants','prizePool','currency','startedAt','endedAt','buyIn','rebuy','prizeOrCashOut','tips','profit','rate','rateDate','note'], ...sessions.map(s => [s.id,s.tripId || '',sessionFormat(s),s.venue,s.location,s.game,s.stakes,s.entrants || '',s.prizePool ? fromMinor(s.prizePool,s.currency) : '',s.currency,s.startedAt,s.endedAt || '',fromMinor(s.buyIn,s.currency),fromMinor(s.rebuy,s.currency),fromMinor(s.cashOut,s.currency),fromMinor(s.tips,s.currency),fromMinor(profit(s),s.currency),s.rate?.rate || '',s.rate?.date || '',s.note])]
...
function SessionForm({ initial, defaultLocation, defaultDate, tripId, tripName, sessions, rate, onClose, onSave }: { initial?: Session; defaultLocation?: string; defaultDate?: string; tripId?: string; tripName?: string; sessions: Session[]; rate: Partial<Record<Currency, Rate>>; onClose: () => void; onSave: (session: Session) => void }) {
  ...
  const [entrants, setEntrants] = useState(initial?.entrants ? String(initial.entrants) : '')
  const [prizePool, setPrizePool] = useState(initial?.prizePool !== undefined ? String(fromMinor(initial.prizePool, initial.currency)) : '')
  ...
  const entrantsNumber = Number(entrants)
  const entryMinor = buyIn !== '' && Number.isFinite(buyInNumber) && buyInNumber >= 0 ? toMinor(buyInNumber, currency) : null
  const prizePoolMinor = prizePool !== '' && Number.isFinite(Number(prizePool)) && Number(prizePool) >= 0 ? toMinor(Number(prizePool), currency) : null
  const estimatedPool = format === 'tournament' && entryMinor !== null && Number.isInteger(entrantsNumber) && entrantsNumber > 0 ? prizePoolMinor ?? entryMinor * entrantsNumber : null
  const averageReturn = estimatedPool !== null ? Math.round(estimatedPool / entrantsNumber) : null
  const averageEv = averageReturn !== null && entryMinor !== null ? averageReturn - entryMinor : null
...
function SessionDetail({ session, currentRate, now, onClose, onEdit, onDelete, onQuickRebuy, onCustomRebuy, onFinish }: { session: Session; currentRate?: Rate; now: number; onClose: () => void; onEdit: () => void; onDelete: () => void; onQuickRebuy: () => void; onCustomRebuy: () => void; onFinish: () => void }) {
  ...
  const invested = session.buyIn + session.rebuy
  const averageReturn = tournamentAverageReturn(session)
  const averageEv = tournamentAverageEv(session)
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className={`modal detail-modal ${active ? 'live-detail-modal' : ''}`} onMouseDown={event => event.stopPropagation()} role="dialog" aria-modal="true" aria-label="セッション詳細">
    ...
    {active ? <>
      ...
      {tournament && averageReturn !== null && <div className="tournament-live-ev"><span>フィールド平均期待払戻</span><strong>{money(averageReturn, session.currency)}</strong><small>1エントリー平均EV {money(averageEv!, session.currency, true)} · 本人の実力差は未反映</small></div>}
    </> : <>
      ...
      <div className="detail-list"><div><span>{tournament ? '初回エントリー' : '初回バイイン'}</span><strong>{money(session.buyIn, session.currency)}</strong></div><div><span>{tournament ? '再エントリー' : 'リバイ・追加購入'}</span><strong>{money(session.rebuy, session.currency)}</strong></div><div><span>{tournament ? '獲得賞金' : 'キャッシュアウト'}</span><strong>{money(session.cashOut, session.currency)}</strong></div><div><span>{tournament ? '追加費用' : 'チップ'}</span><strong>{money(session.tips, session.currency)}</strong></div>{tournament && session.entrants && <div><span>総エントリー数</span><strong>{session.entrants.toLocaleString('ja-JP')}</strong></div>}{tournament && averageReturn !== null && <><div><span>平均期待払戻</span><strong>{money(averageReturn, session.currency)}</strong></div><div><span>フィールド平均EV</span><strong>{money(averageEv!, session.currency, true)}</strong></div></>}<div><span>プレー時間</span><strong>{duration(session, now)}</strong></div></div>
