...
export const loadRemote = async (uid: string) => {
  if (!db) return { sessions: [] as Session[], players: [] as Player[] }
  const [sessions, players] = await Promise.all([getDocs(collection(db, 'users', uid, 'sessions')), getDocs(collection(db, 'users', uid, 'players'))])
  return { sessions: sessions.docs.map(item => item.data() as Session), players: players.docs.map(item => item.data() as Player) }
}
